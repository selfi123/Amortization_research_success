...
PROCESS_THREAD(sender_process, ev, data)
{
    ...
    /* Z1: Buffer sized to match reassembly_buf in gateway (640B).
     * Max payload = 1+12+128+256+128+32+32 = 589 bytes with POLY_DEGREE=32 */
    static uint8_t serialized_buffer[640];
    size_t offset = 0;
    ...
    for (frag_idx = 0; frag_idx < total_frags; frag_idx++) {
        /* Z1: 'attempts' must be static to survive PROCESS_YIELD (setjmp boundary) */
        static int attempts;
        attempts = 0;
        int acked = 0;
