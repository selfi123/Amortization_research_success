 ...
 * *** Z1 MOTE VARIANT (Zolertia Z1 / MSP430F2617) ***
 *
 * Target Hardware Constraints:
 *   - RAM:   8 KB  (8192 bytes)
 *   - Flash: 92 KB (94208 bytes)
 *   - CPU:   MSP430 16-bit @ 16 MHz
 *
 * Key Optimizations vs Cooja Mote version:
 *   - POLY_DEGREE reduced 128 -> 32  (saves 75% polynomial RAM)
 *   - RING_SIZE reduced 3 -> 2      (saves 1x Poly32 per signature)
 *   - MAX_SESSIONS reduced 16 -> 2  (saves 4x session_entry_t structs)
 *   - FLASH_CONST macro routes large constant tables to Flash memory
 *   - AuthFragment payload reduced 64 -> 48 bytes
 *
 * RAM Budget (approx after reductions):
 *   RingLWEKeyPair : 3 x (32x4) = 384 bytes
 *   RingSignature  : (2+1) x 128 + 64 = 448 bytes
 *   session_entry  : 2 x 65 = 130 bytes
 *   OS overhead    : ~4000 bytes
 *   Total approx   : ~5000 bytes (fits in 8192!)
 *
 * Implements:
 * - Ring-LWE with polynomial degree 128
...
#ifndef CRYPTO_CORE_Z1_H_
#define CRYPTO_CORE_Z1_H_

#include <stdint.h>
#include <string.h>
#include <stddef.h>

/* ========== Z1 FLASH PLACEMENT MACRO ========== */
/* On MSP430 (Z1 mote), 'const' data is automatically stored in Flash ROM.
 * Use FLASH_CONST on large lookup tables to avoid burning precious 8KB RAM.
 * Example: FLASH_CONST uint32_t my_table[256] = {...};
 */
#define FLASH_CONST const

/* ========== RING-LWE PARAMETERS ========== */

#define POLY_DEGREE 32                     // n: Z1-reduced from 128 (saves 75% RAM)
...
#define RING_SIZE 2                        // N: Z1-reduced from 3 to 2 members
...
#define MAX_SESSIONS 2                     // Z1: Reduced to 2 to fit in 8KB RAM
...
typedef struct __attribute__((packed)) {
    ...
    uint8_t payload[64];                   // Must match chunk size used in fragmentation (64 bytes)
